...
**推荐使用 Docker Compose 部署 Web 版。** 它会自动帮你配置访问令牌、容器网络
和持久化数据卷，省去手动运行 `node server/index.cjs` 时繁琐且容易遗漏的安全
配置步骤。
...
Web 网关要求访问令牌。每个请求和 WebSocket 连接都必须携带有效会话才能访问
SSH/SFTP 接口；未鉴权的访客只会看到登录页，而不是应用本身。

- **自定义令牌** —— 设置 `AI_SSH_CLIENT_WEB_TOKEN`（会以 `WEB_AUTH_TOKEN`
  传入容器）。任何多人共享的部署都建议这样做：

  ```bash
  AI_SSH_CLIENT_WEB_TOKEN='一个足够长的随机密钥' docker compose up -d --build
  ```

- **自动生成令牌** —— 若不设置，网关会在首次启动时生成随机令牌，持久化到数据
  卷，并打印到容器日志。可通过以下命令获取：

  ```bash
  docker compose logs ai-ssh-client-web | grep "Web access token"
  ```

打开页面，输入一次令牌，之后会话 Cookie 会保持登录状态。
...
- **访问令牌是以明文经 HTTP 传输的。** 在局域网内通常可以接受，但只要流量可能
...
> 使用保存的凭据发起 SSH 连接。访问令牌能提供保护，但在明文 HTTP 下该令牌可能
> 被截获。任何面向公网的部署都请置于 HTTPS 之后。详见
